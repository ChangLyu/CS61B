...
public class Sobel {
  ...
  private static void sobelFile(String filename, int numIterations,
   ...
   // change it by myself
      ...
      String frontAddress=blurname.substring(0,blurname.lastIndexOf("/")+1);
      String lastAppendix=blurname.substring(blurname.indexOf("."),blurname.length()-1);
      blurname=frontAddress+"blur_"+blurname.substring(blurname.lastIndexOf("/")+1,blurname.indexOf("."))+lastAppendix;
      // the code before changed is add the blur_ directly to the name of file, which does not take the address as consideration.
    ...
    String edgename = filename;
    String frontAddress=edgename.substring(0,edgename.lastIndexOf("/")+1);
    String lastAppendix=edgename.substring(edgename.indexOf("."),edgename.length()-1);
    edgename=frontAddress+"edge_"+edgename.substring(edgename.lastIndexOf("/")+1,edgename.indexOf("."))+lastAppendix;
    // the code before changed is add the blur_ directly to the name of file, which does not take the address as consideration.
    System.out.println("Writing grayscale-edge image file " + edgename);
    TIFFEncoder.writeTIFF(sobeled, edgename);
    if (rle) {
    	// change it by myself
        String rlename = filename;
        String frontAddress2=rlename.substring(0,rlename.lastIndexOf("/")+1);
        String lastAppendix2=rlename.substring(rlename.indexOf("."),rlename.length()-1);
        rlename=frontAddress2+"blur_"+rlename.substring(rlename.lastIndexOf("/")+1,rlename.indexOf("."))+lastAppendix2;
        // the code before changed is add the blur_ directly to the name of file, which does not take the address as consideration.
        
      String rlename2 = "rle_" + filename;
